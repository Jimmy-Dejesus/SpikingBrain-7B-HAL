...
"""Attention layer for Meta-FlashAttention."""
...
from .fa_kernel_metatoken import metatoken_flash_attn_with_kvcache, metatoken_flash_attn_varlen_func
...
class MetaAttentionBackend(AttentionBackend):
    ...
    @staticmethod
    def get_name() -> str:
        return "META_FLASH_ATTN"
    
    @staticmethod
    def get_impl_cls() -> Type["MetaAttentionImpl"]:
        return MetaAttentionImpl
    
    @staticmethod
    def get_metadata_cls() -> Type["MetaAttentionMetadata"]:
        return MetaAttentionMetadata
    
    @staticmethod
    def get_builder_cls() -> Type["MetaAttentionMetadataBuilder"]:
        return MetaAttentionMetadataBuilder
    ...
    def get_kv_cache_shape(
        ...
        if block_size % 16 != 0 or block_size < 16:
            raise ValueError(
                f"Invalid block size {block_size}. Must be a multiple of 16 and at least 16."
            )
    ...
    def swap_blocks(
        ...
        ops.swap_blocks(
            src_key_cache, dst_key_cache, scr_to_dst
        )
        src_value_cache = src_kv_cache[1]
        dst_value_cache = dst_kv_cache[1]
        ops.swap_blocks(
            src_value_cache, dst_value_cache, scr_to_dst
        )
    ...
    def copy_blocks(
        ...
        ops.copy_blocks(
            key_caches, value_caches, src_to_dists
        )
    
@dataclass
class MetaAttentionMetadata(AttentionMetadata):
    ...
    _cached_prefill_metadata: Optional["MetaAttentionMetadata"] = None
    _cached_decode_metadata: Optional["MetaAttentionMetadata"] = None
    ...
    @property
    def prefill_metadata(self) -> Optional["MetaAttentionMetadata"]:
        """
        ...
        # If the prefill metadata is not cached, create it.
        ...
        self._cached_prefill_metadata = MetaAttentionMetadata(
    ...
    @property
    def decode_metadata(self) -> Optional["MetaAttentionMetadata"]:
        ...
        self._cached_decode_metadata = MetaAttentionMetadata(
            num_prefills=0,
...
class MetaAttentionImpl(AttentionImpl):
    """
    ...
    And for MetaAttention, all the input started with meta tokens, for example,
    |<-----num_meta_tokens----->|<----- num_prefill_tokens ---->|<------- num_decode_tokens --------->|
    |<-meta_0->|...|<-meta_K-1->|<-prefill_0->|...|<-prefill_N-1->|<--decode_0-->|...|<--decode_M-1-->|
    """
    def __init__(
        self,
        num_heads: int,
        ...
        if kv_sharing_target_layer_name is not None:
            raise NotImplementedError("KV sharing is not supported in V0 MetaAttention.")
        if blocksparse_params is not None:
            raise ValueError("MetaAttention does not support blocksparse attention.")
        if use_irope:
            logger.warning(
        ...
        if is_quantized_kv_cache(kv_cache_dtype) and (
            ...
            raise NotImplementedError(
                f"MetaAttention does not support {self.kv_cache_dtype} "
                "kv-cache on this device "
                f"(FA supports fp8 = {flash_attn_supports_fp8()}).")
        if logits_soft_cap is None:
            # logits_soft_cap means the maximum value of logits.
        ...
        support_head_sizes = MetaAttentionBackend.get_supported_head_sizes()
        if self.head_size not in support_head_sizes:
            raise ValueError(
                f"Head size {head_size} is not supported by MetaAttention. "
    ...
    def forward(
        ...
        value2: Optional[torch.Tensor],
        kv_cache: torch.Tensor,
        attn_metadata: MetaAttentionMetadata,
        output: Optional[torch.Tensor] = None,
        output_scale: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """Forward pass with MetaAttention.
        ...
        Args:
            ...
            kv_cache: shape = [2, num_blocks, block_size, num_kv_heads, head_size]
                ...
                for profiling run.
            attn_metadata: MetaAttentionMetadata object containing metadata
        ...
        if output_scale is not None:
            raise NotImplementedError(
                "fused output quantization is not supported in MetaAttention.")
        ...
        if fp8_attention and not flash_attn_supports_fp8():
            raise NotImplementedError(
                "MetaAttention does not support fp8 kv-cache on this device "
                f"(FA supports fp8 = {flash_attn_supports_fp8()}).")
        
        if kv_cache.numel() > 0: # kv_cache is not empty
        ...
        if prefill_meta := attn_metadata.prefill_metadata:
            ...
            if (kv_cache.numel() == 0 or prefill_meta.block_tables is None
                ...
                # 性能分析运行 或者 Encoder 推理，所以没有保存 kv cache
                # normal attention
                ...
                if fp8_attention:
                    num_key_tokens, num_kv_heads, head_size = key.shape

                    key, _ = ops.scaled_fp8_quant(
                        key.reshape(
                            (num_key_tokens, num_kv_heads * head_size)
                        ).contiguous(),
                        layer._k_scale)
                    key = key.reshape((num_key_tokens, num_kv_heads, head_size))
                ...
                # 当一个新请求的前一部分内容之前已经被处理并缓存时触发。
                # 例如，在多轮对话中，前面的对话历史就可以作为前缀被缓存和复用。
        ...
        if decode_meta := attn_metadata.decode_metadata:
            ...
            if decode_meta.max_decode_query_len > 1:
                ...
                # flash_attn_with_kvcache 这个内核被设计出来，
                # 就是为了解决自回归解码中最经典、最耗时的那个瓶颈：每一步只生成一个 token。
                # 不过由于前面已经更新了 kv cache 所以这里不传入 key 和 value 避免二次更新
                (
                    seq_lens_arg,
                    _,
                    block_tables_arg,
